...
install_go() {
    ...
    wget https://go.dev/dl/${latest_go}.linux-${caddy_arch}.tar.gz
    tar -zxf ${latest_go}.linux-${caddy_arch}.tar.gz -C /usr/local/
...
install_caddy() {
    ...
    latest_release=$(curl -s https://api.github.com/repos/klzgrad/forwardproxy/releases/latest | grep browser_download_url | grep caddy-forwardproxy-naive.tar.xz | cut -d '"' -f 4)
    wget $latest_release -O caddy-forwardproxy-naive.tar.xz
